...
f.init(conf.plc_ip,conf.plc_port,conf.plc_station,function(err,mod) {	
    if(err) {
	console.log(util.format("Chyba:%j",err))
	return
    }
    fatek = mod
...
function parseLoop(bfr) {
    ...
    if(ack!=06) return console.error(ch.red('Failed ACK'))
...
*/
	...
	Math.round(wind_spd), //R906
	Math.round(wind_avg), //R907
